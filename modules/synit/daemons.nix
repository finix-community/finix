...
let
  ...
  daemonToPreserves =
    ...
    let
      ...
      protocol =
        if hasReadyOnNotify then
          assert attrs.protocol == "none";
          "application/syndicate"
        else
          attrs.protocol;
      attrs' = {
        argv =
          ...
          ++
            ...
            optionals (attrs.logging.enable || hasReadyOnNotify) [
              "emptyenv"
              "-c"
            ]
          ++ optionals (attrs.logging.enable || hasReadyOnNotify) [
          ...
          ++ optionals (attrs.logging.enable && attrs.syd.enable) (quoteExecline [
            ...
            "pipeline"
            "-w"
            ([ "s6-log" ] ++ attrs.logging.args ++ [ "${attrs.logging.dir}.syd" ])
            "fdswap"
            "1"
            "$SYD_LOG_FD"
            "export"
        ...
        [
          ''
            ? <run-service <daemon ${name}>> [
              ! <exec ${
                let
                  dst = "/run/synit/config/persistent/daemon-${name}.pr";
                in
                [
                  "if"
                  "-t"
...
in
{
  ...
  config = mkIfSynit {
    ...
    synit.plan.config = mapAttrs' (name: value: {
      name = "daemon-${name}";
      value = daemonToPreserves name value;
    }) cfg.daemons;
  };
