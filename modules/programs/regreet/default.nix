...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.greetd.settings = {
      default_session = {
        command = "env XDG_DATA_DIRS=/etc ${lib.getExe pkgs.cage} -s -m last -- ${lib.getExe pkgs.greetd.regreet} --config ${configFile}" + lib.optionalString cfg.debug " --log-level debug";
      };
    };
