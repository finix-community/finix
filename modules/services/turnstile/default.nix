...
in
{
  options.services.turnstile = {
    enable = lib.mkOption {
      ...
      default = false;
      description = ''
        Whether to enable [turnstilel](${pkgs.turnstile.meta.homepage}) as a system service.
      '';
    };
    ...
    type =
      ...
      attrsOf (submodule {
        config = lib.mkIf cfg.enable {
          conditions = "service/turnstiled/ready";
        };
      });
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    security.pam.services = {
      turnstiled.text = ''
        ...
        ${lib.optionalString config.services.sessiond.enable "session optional ${config.services.sessiond.package}/lib/security/pam_sessiond.so"}
