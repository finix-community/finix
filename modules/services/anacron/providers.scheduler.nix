{ config, lib, ... }:
let
  anacronCompatible = v: v.interval == "daily" || v.interval == "weekly" || v.interval == "monthly" || v.interval == "yearly";
in
{
  ...
  config = lib.mkIf (config.providers.scheduler.backend == "anacron") {
    ...
    services.cron.systab = lib.mapAttrsToList (k: v:
      let
        expr =
          if v.interval == "hourly" then "0 0 * * *"
          else v.interval
        ;
    ...
    services.anacron.systab = lib.mapAttrsToList (k: v:
      "@${v.interval} 0 ${k} ${v.command}"
